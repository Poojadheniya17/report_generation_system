...
report = {
    ...
    "triad": {
        "employee_snapshot": {
            "text": "Jordan's strongest tendency is Sociability, echoed by high Agreeableness and Open-Mindedness in the personality results, which together predict a team member who builds trust quickly and keeps collaboration genuinely two-way rather than transactional. The moderate task orientation means Jordan follows through reliably without needing rigid process to stay on track, and pairs naturally with the relational strength rather than competing with it. Dominance is only mildly present, so Jordan is likely to lead through consistent engagement and invitation rather than assertiveness, adding the most value in roles that depend on cross-functional coordination and steady presence."
        },
        "task": {
